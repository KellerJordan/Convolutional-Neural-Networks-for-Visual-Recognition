...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        scale = 1 / np.sqrt(x_var + eps)
        x_norm = (x - x_mean) * scale
        out = gamma * x_norm + beta
        cache = (x_norm, scale, gamma)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # half solved formula
    x_norm, scale, gamma = cache
    N = x_norm.shape[0]
    dx_norm = gamma * dout
    dz = (dx_norm - (x_norm/N)*np.sum(dx_norm * x_norm, axis=0)) * scale
    dx = dz - (1/N)*np.sum(dz, axis=0)
    
    dgamma = (x_norm * dout).sum(axis=0)
    dbeta = dout.copy().sum(axis=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    xn, scale, gamma = cache
    N = xn.shape[0]
    dxn = gamma * dout
    dx = scale*dxn - (scale/N)*(dxn.sum(axis=0) + xn*(dxn*xn).sum(axis=0))
#     dx = scale/N * (N*dxn - dxn.sum(axis=0) - xn*(dxn*xn).sum(axis=0))
    dgamma = (xn * dout).sum(axis=0)
    dbeta = dout.copy().sum(axis=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_bnflat = x.transpose(0, 2, 3, 1).reshape((N*H*W, C))
    out_bnflat, cache = batchnorm_forward(x_bnflat, gamma, beta, bn_param)
    out = out_bnflat.reshape(N, H, W, C).transpose(0, 3, 1, 2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dout_bnflat = dout.transpose(0, 2, 3, 1).reshape((N*H*W, C))
    dx_bnflat, dgamma, dbeta = batchnorm_backward(dout_bnflat, cache)
    dx = dx_bnflat.reshape(N, H, W, C).transpose(0, 3, 1, 2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
